...
import { MarketStateV1, MarketV1Data } from "./types";
...
export const getUtilization = (marketState: MarketStateV1) => {
  return wDivDown(marketState.totalBorrowAssets, marketState.totalSupplyAssets);
};

function getWithdrawalToUtilization(marketState: MarketStateV1, targetUtilization: bigint) {
  return wMulDown(
...
function getDepositToUtilization(marketState: MarketStateV1, targetUtilization: bigint) {
  return wMulDown(
    marketState.totalSupplyAssets,
